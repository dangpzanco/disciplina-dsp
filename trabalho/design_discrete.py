...
def plot_zpk(system, fp, fs, Amax, Amin, sample_rate=48e3, num_samples=1024, ax=None):
    ...
    # Set plot properties
    ax.set_title('Lowpass filter fit to constraints')
    ax.set_xlabel('Frequency [Hz]')
    ax.set_ylabel('Amplitude [dB]')
    ax.grid(True, which='both', axis='both')
    ax.axis([1e2, 10**fmax, -50, 1])
...
def optimize_filter(spec, min_order=4, filter_type='but', method='zoh', num_samples=100, limits_samples=1000):
    ...
    Amax_vec = np.random.uniform(1e-3, spec['Amax'], num_samples)
    Amin_vec = np.random.uniform(spec['Amin'], 2*spec['Amin'], num_samples)
...
analog_system, discrete_system, final_spec = optimize_filter(spec, filter_type='but', method='zoh', num_samples=1000)
print(final_spec)
