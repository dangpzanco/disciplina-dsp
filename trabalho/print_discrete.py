...
def print_discrete(b, a):
    ...
    if num_b < num_a:
        b = np.pad(b, (num_a-num_b, 0), 'constant')

    print('')
    for i in range(num_a):
        print(f'$z^{num_a-i-1}$ & {b[i]:.4e} & {a[i]:.4e}')
...
for i, filter_type in enumerate(filter_list):
    for j, method in enumerate(method_list):
        ...
        discrete_b, discrete_a = signal.zpk2tf(*discrete_system)
        ...
        print('Coefficients:')
        # print_discrete(discrete_b, discrete_a)
        print_analog(analog_b, analog_a)
        # print(f'{Amax_final:.4f}\n{Amin_final:.4f}')


        # print(discrete_system[1].size)
        # print(sos.shape[0])
